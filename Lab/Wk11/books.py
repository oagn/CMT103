...
def get_book_title(db, ID):
    cursor = db.cursor()
    cursor.execute("SELECT title FROM books WHERE id="+str(ID))
    db.commit()
    record =cursor.fetchone()
    if not record:
        return  None
...
def get_book_year(db, ID):
    cursor = db.cursor()
    cursor.execute("SELECT year FROM books WHERE id="+str(ID))
    db.commit()
    record =cursor.fetchone()
    if not record:
        return  None
...
def get_book_author(db, ID):
    cursor = db.cursor()
    cursor.execute("SELECT author_id FROM books WHERE id="+str(ID))
    db.commit()
    record =cursor.fetchone()
    if not record:
        return  None
...
def update_book(db, ID):
    cursor = db.cursor()
    titl =get_book_title(db, ID)
    if not titl:
        print ("No such book.")
        return
    title=input("Change name ("+get_book_title(db, ID)+") to: ")
    year=input("Change year ("+get_book_year(db, ID)+") to: ")
    cursor.execute("SELECT books.id, books.author_id, authors.name FROM books JOIN authors ON books.author_id=authors.id WHERE books.id="+str(ID))
    db.commit()
    records = cursor.fetchone()
    cursor.close()
    author=input("Change author ("+records[2]+") to: ")
    authorID = get_author_ID(db, author)
    if not authorID:
        add_author(db, author)
    authorID = get_author_ID(db, author)

    cursor.execute("UPDATE books SET (title, year, author_id) VALUES ('"+title+"', "+str(year)+", "+str(authorID)+")' WHERE id="+str(ID))
    db.commit()
    cursor.close()
